...
const LeadInSchema = z.object({
    ...
    perfil_psico: z.string().optional().nullable(),
...
export async function POST(req: Request) {
    try {
        ...
        const parsed: LeadIn = LeadInSchema.parse(body);
        ...
        if (!parsed.landing_id && !parsed.public_hash) {
            return NextResponse.json(
                { error: "landing_id ou public_hash obrigatório." },
                { status: 400 }
            );
        }
        ...
        const prazoMesesIn =
            parsed.prazoMeses ?? (parsed.prazo_meses != null ? Number(parsed.prazo_meses) : null);
        ...
        if (hasInterest) {
            const { error: intErr } = await s.from("lead_interesses").insert({
                org_id: landing.org_id,
                lead_id: leadId,
                produto: parsed.tipo ?? null,
                valor_total: valorInteresseIn as number | null,
        ...
        console.error("POST /api/leads/create error:", e);
