...
type Props = {
    cotaId: string;
    initialValues: ModalidadesInitialValues;
    onDirtyChange?: (dirty: boolean) => void;
};

function buildDefaultOptions(
    values: ModalidadesInitialValues["opcoesLanceFixo"]
    ...
    return [1, 2, 3].map((ordem) => {
        ...
        return {
            ...
            ordem,
            percentual:
                existing?.percentual == null ? null : String(existing.percentual),
...
export function CartaModalidadesForm({
                                         cotaId,
                                         initialValues,
                                         onDirtyChange,
                                     }: Props) {
    ...
    const defaultValues = useMemo<CartaModalidadesFormValues>(
        () => ({
            embutidoPermitido: !!initialValues.embutidoPermitido,
            embutidoMaxPercent:
                initialValues.embutidoMaxPercent == null
                    ? null
                    : String(initialValues.embutidoMaxPercent),
            fgtsPermitido: !!initialValues.fgtsPermitido,
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid gap-4">
                        ...
                        <CardContent className="space-y-4">
                                ...
                                render={({ field }) => (
                                    <FormItem className="flex items-center justify-between rounded-xl border border-white/10 bg-black/10 p-4">
                                        <div className="space-y-1">
                                            ...
                                            <FormDescription>
                                                Use apenas quando a carta e a operação admitirem esta modalidade.
                                            </FormDescription>
                                        </div>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormDescription>
                                            Percentual máximo da carta que pode ser usado no embutido.
                                        </FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            ...
                            {embutidoPermitido ? (
                                <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-sm text-muted-foreground">
                                    Embutido habilitado. Revise esse limite com cuidado para não comprometer o crédito útil.
                                </div>
                            ) : (
                                ...
                                render={({ field }) => (
                                    <FormItem className="flex items-center justify-between rounded-xl border border-white/10 bg-black/10 p-4">
                                        <div className="space-y-1">
                                            ...
                                            <FormDescription>
                                                Habilite somente quando a operação estiver alinhada às regras aplicáveis.
                                            </FormDescription>
                                        </div>
                    ...
                    <Card className="border-white/10 bg-white/5">
                        ...
                        <CardContent className="space-y-4">
                            <div className="rounded-xl border border-white/10 bg-black/10 p-3 text-sm text-muted-foreground">
                                Cadastre até 3 opções. Ative somente as que realmente fazem sentido para esta carta.
                            </div>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="mt-4">
                                                            <FormField
                                                                control={form.control}
                                                                name={`opcoesLanceFixo.${index}.observacoes`}
                                                                render={({ field }) => (
                                                                    <FormItem>
                ...
                <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t bg-background/95 pt-4 backdrop-blur">
                    <div className="text-xs text-muted-foreground">
                        Salve apenas esta aba para atualizar modalidades, embutido, FGTS e opções fixas.
                    </div>
