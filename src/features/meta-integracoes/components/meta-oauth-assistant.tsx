...
function buildDefaultIntegrationName(pageName?: string | null) {
  return pageName ? `Meta ${pageName}` : "";
}
...
export function MetaOAuthAssistant({
  ...
  const [defaultOwnerId, setDefaultOwnerId] = useState<string>(NONE_VALUE);

  const [nome, setNome] = useState("");
  const [sourceLabel, setSourceLabel] = useState("Meta Ads");

  const [nomeTouched, setNomeTouched] = useState(false);
  const [sourceLabelTouched, setSourceLabelTouched] = useState(false);
  ...
  const applyPageSelection = useCallback(
      (pageId: string, nextPages: MetaPage[] = pages) => {
        if (pageId === NONE_VALUE || !pageId) {
          setSelectedPageId("");
          setSelectedFormId("");
          setForms([]);
          return;
        ...
        if (!nomeTouched) {
          setNome(buildDefaultIntegrationName(page?.name));
        }

        if (!sourceLabelTouched) {
          setSourceLabel(page?.name ?? "Meta Ads");
        }

        loadForms(pageId);
      },
      [loadForms, nomeTouched, pages, sourceLabelTouched],
  );
  ...
  const loadPages = useCallback(() => {
    startTransition(async () => {
      try {
        ...
        applyPageSelection(firstPage.id, nextPages);
      } catch (error) {
        setPages([]);
        setForms([]);
        setSelectedPageId("");
  ...
  useEffect(() => {
    if (!oauthSuccess || oauthSuccessHandledRef.current) {
      return;
    }
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Select
                  ...
                  onValueChange={(value) => applyPageSelection(value)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Selecione uma página" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={NONE_VALUE}>Selecione uma página</SelectItem>
                  {pages.map((page) => (
            ...
            <div className="grid gap-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value={NONE_VALUE}>Todos os formulários</SelectItem>
                  {forms.map((form) => (
            ...
            <div className="grid gap-2">
              <Label htmlFor="oauth-source-label">Nome da origem do lead</Label>
              <Input
                  id="oauth-source-label"
                  value={sourceLabel}
                  onChange={(event) => {
                    setSourceLabelTouched(true);
            ...
            <div className="grid gap-2 md:col-span-2">
              ...
              <Select value={defaultOwnerId} onValueChange={setDefaultOwnerId}>
                ...
                <SelectContent>
                  <SelectItem value={NONE_VALUE}>Sem responsável padrão</SelectItem>
          ...
          <div className="rounded-2xl border border-emerald-400/12 bg-emerald-400/[0.05] p-4 text-sm text-slate-200">
            ...
            <div className="space-y-1 text-sm text-slate-300">
              ...
              <p>
                Formulário: {selectedForm?.name ?? "Todos os formulários"}
              </p>
              <p>Nome interno: {nome || "—"}</p>
              <p>Nome da origem: {sourceLabel || "—"}</p>
            </div>
          </div>
                    ...
                    startTransition(async () => {
                      try {
                        const integration = await finalizeMetaOAuthIntegrationAction({
                          ...
                          default_owner_id:
                              defaultOwnerId === NONE_VALUE ? null : defaultOwnerId,
                          ativo: true,
                        });
